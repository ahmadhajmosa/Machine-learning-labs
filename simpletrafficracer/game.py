...
training_data = []
# Train the model on the recorded game states
def train():
    for state, action, reward, next_state, done, x_t, movedL, movedU in training_data:
        actions_costs = np.array([np.zeros([4])])
        actions_costs[0][action] = 1  # set the actions to the ones used in the recording
        model.fit(np.reshape(state,(1,80,80,12)), actions_costs, epochs=1, verbose=0)


# load all save files (numbered from 1.dat)
num_file = 1
while os.path.isfile('./data/' + str(num_file) + '.dat'):
    infile = open('data/' + str(num_file) + '.dat','rb')
...
def terminate():
    ...
    # save model on termination and save the results

    model.save("model.h5")


    g = open("data/times.dat", 'w')
    for sv in save_time:
        g.write(sv + "\n")
    g.close()
...
while (count>0):
    ...
    while True: # the game loop
        ...
        #Black out the edges to ignore the moving grass
        x_t[:120,:]=(0,0,0)
        x_t[505:,:]=(0,0,0)
        ...
        # Try to constrict the agent to try to keep the car in the middle and bottom of the road
        cent_x, cent_y = playerRect.center
        r_t = - abs(312 - cent_x)/2000.0  # 312 is the middle of the road
        r_t += cent_y/1000.0
    ...
    # write and save scores
    print (t,int(time.time() - start_time), score)

    save_time.append(str (t) + str(int(time.time() - start_time)) + " " + str(score))
