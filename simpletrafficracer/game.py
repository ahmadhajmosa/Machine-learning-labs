    ...
    for i in xrange(0,4):
        for j in xrange(0,80):
            for k in xrange(0,80):
                str=str+st[k,j,i].astype('string')+","
            str = str[:-1]
            str+="~"
        str = str[:-1]
        str+="#"
...
def replay(batch_size):
        ...
        for state, action, reward, next_state, done in minibatch:   #done true if player hit baddie
            target = reward
            if not done:
             target = reward + gamma * \
                       np.amax(model.predict(np.reshape(next_state,(1,80,80,4)))[0])
            target_f = model.predict(np.reshape(state,(1,80,80,4)))
            target_f[0][action] = target
            model.fit(np.reshape(state,(1,80,80,4)), target_f, epochs=1, verbose=0)
...
if(build_memory):
    file=open("memory","w+")
if(use_memory and Path("memory").is_file()):
    print("Loading memory file...")
    file=open("memory","r")
    line=file.readline()
    while line:
        content=line.split("|")
        s_t_c=convert_array(content[0])
        s_t_c=np.reshape(s_t_c,(80,80,4))
        s_t_c=s_t_c.reshape(80,80,4)
        s_t1_c=convert_array(content[3])
        s_t1_c=np.reshape(s_t1_c,(80,80,4))
        s_t1_c=s_t1_c.reshape(80,80,4)
        transform_c=False
        if(content[4]=="True"):
            transform_c=True
        memory.append((s_t_c,int(content[1]),int(content[2]),s_t1_c,transform_c))
        line=file.readline()
    file.close()
    s_t=s_t_c
    t=5001
...
while (count>0):
    ...
    while True: # the game loop
        ...
        if playerHasHitBaddie(playerRect, baddies):
            ...
            if(build_memory and t<=OBSERVE):
                file.write("%s|%s|%s|%s|%s\n" %(print_array(s_t),action_index,r_t,print_array(s_t1),terminal))
            if score > topScore:
