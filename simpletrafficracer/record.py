...
def save_game():
    ...
    # Save the game states to the next avaialble file name

    file_num = 1
    while os.path.isfile('./data/' + str(file_num) + '.dat'):
        file_num += 1
    filename = './data/' + str(file_num) + '.dat'

    outfile = open(filename,'wb')

    pickle.dump(saved_games,outfile)
    outfile.close()
...
while (count>0):
    ...
    while True: # the game loop
        ...
        # Insert manaul controls to the car
...
#Brought to you by code-projects.org
        ...
        # Save the state if the car has not crashed

        if terminal == False:
            saved_games.append((s_t, action_index, r_t, s_t1, terminal, x_t, movedLeft, movedUp))
