...
def build_model(shape, export_graphstructue=True):
    ...
    input_layer = tf.keras.Input(shape=(shape[0], shape[1]), dtype="float16", name="input_layer")
    reshaped = kl.Reshape(target_shape=(shape[0], shape[1], 1), name="reshaper")(input_layer)
    conv1 = kl.Conv2D(filters=32, kernel_size=(8,8), data_format="channels_last", padding="valid")(reshaped)
    ...
    left_right_dense = kl.Dense(128, activation="relu", name="left_right_dense")(flat)
    forward_backward_dense = kl.Dense(128, activation="relu", name="forward_backward_dense")(flat)
    ...
    left_right_logits = kl.Dense(steer_actions, name="left_right_logits")(left_right_dense)
    forward_backward_logits = kl.Dense(gas_actions, name="forward_backward_logits")(forward_backward_dense)
    ...
    model = tf.keras.Model(inputs=[input_layer], outputs=[left_right_logits, forward_backward_logits])
...
class A2CAgent:
    ...
    def train(self, env, batch_sz=64, updates=1000):
        ...
        observations = np.empty((batch_sz,) + (env.max_diff_x*2,) + (env.max_diff_y*2,))
        ...
        for update in tqdm(range(updates)):
            for step in range(batch_sz):
                ...
                actions[step], values[step] = map(lambda x: x.value, action_value(self.model, edit_ops[None, :]))
                dones[step] = env.next_round(LeftRight(actions[step]), FrontBack(values[step]))
            ...
            _, next_value = action_value(self.model, next_obs[None, :]) # TODO: Look at this and the next line!!!!, look http://inoryy.com/post/tensorflow2-deep-reinforcement-learning/
...
env = GameDing()
model = build_model(shape=(env.max_diff_x*2, env.max_diff_y*2), export_graphstructue=True)
