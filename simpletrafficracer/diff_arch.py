...
def replay(batch_size):
        ...
        for state, action, reward, next_state, done in minibatch:
            ...
            if not done:
              target = reward + gamma * \
                       np.amax(model.predict(np.reshape(next_state,(1,16,27,12)))[0])

            target_f = model.predict(np.reshape(state,(1,16,27,12)))
            target_f[0][action] = target
            model.fit(np.reshape(state,(1,16,27,12)), target_f, epochs=1, verbose=0)


# Different model architecture, non-cnn based
...
def build_second_model():
    ...
    model.add(Flatten())
    # 1st Dense Layer
    model.add(Dense(500, input_shape=(432,4*3)))
...
while (count>0):
    ...
    while True: # the game loop
        ...
        img = img[4:-10,3:]  #16x27
        x_t = img


        if t==0:
            s_t = np.stack((x_t, x_t, x_t, x_t), axis=2).reshape(16,27,12)
            s_t1 = s_t
        else:
            x_t = np.reshape(x_t,(16,27,3))
            s_t1 = np.append(x_t, s_t[:,:,0:9], axis = 2).reshape(16,27,12)
        ...
        if random.random() <= epsilon or t <= OBSERVE:
            ...
            readout_t = model.predict(np.reshape(s_t,(1,16,27,12)))
